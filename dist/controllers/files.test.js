...
vitest_1.vi.mock('../services', () => {
    ...
    FilesService.prototype.uploadOne = vitest_1.vi.fn();
    const MetaService = vitest_1.vi.fn();
    MetaService.prototype.getMetaForQuery = vitest_1.vi.fn().mockResolvedValue({});
    return { FilesService };
});
(0, vitest_1.describe)('multipartHandler', () => {
    (0, vitest_1.it)(`Errors out if request doesn't contain any files to upload`, () => {
        const fakeForm = new form_data_1.default();
        fakeForm.append('field', 'test');
        const req = {
            headers: fakeForm.getHeaders(),
            is: vitest_1.vi.fn().mockReturnValue(true),
        ...
        (0, files_1.multipartHandler)(req, res, (err) => {
            (0, vitest_1.expect)(err.message).toBe('No files where included in the body');
            (0, vitest_1.expect)(err).toBeInstanceOf(invalid_payload_1.InvalidPayloadException);
        });
    });
    (0, vitest_1.it)(`Errors out if uploaded file doesn't include the filename`, () => {
        const fakeForm = new form_data_1.default();
        ...
        stream.push(fakeForm.getBuffer());
        (0, files_1.multipartHandler)(req, res, (err) => {
            (0, vitest_1.expect)(err.message).toBe('File is missing filename');
            (0, vitest_1.expect)(err).toBeInstanceOf(invalid_payload_1.InvalidPayloadException);
        });
    });
});
