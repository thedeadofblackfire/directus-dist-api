...
(0, vitest_1.describe)('Integration Tests', () => {
    ...
    (0, vitest_1.describe)('Services / Flows', () => {
        let service;
        let flowManagerReloadSpy;
        (0, vitest_1.beforeEach)(() => {
            service = new _1.FlowsService({
                knex: db,
                schema: {
                    collections: {
                        directus_flows: {
        ...
        (0, vitest_1.describe)('deleteOne', () => {
            (0, vitest_1.it)('should update operations once and reload flows once', async () => {
                await service.deleteOne(1);
                const updateOperationsCalls = tracker.history.update.filter((query) => query.sql.includes(`update "directus_operations" set "resolve" = ?, "reject" = ? where "flow"`)).length;
                (0, vitest_1.expect)(updateOperationsCalls).toBe(1);
                (0, vitest_1.expect)(flowManagerReloadSpy).toBeCalledTimes(1);
            });
        });
