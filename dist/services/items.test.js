...
describe('Integration Tests', () => {
    ...
    describe('createOne', () => {
        ...
        it.each(Object.keys(schemas))(`%s creates one item in collection as an admin, accountability: "null"`, async (schema) => {
            ...
            expect(tracker.history.insert[0].sql).toBe(`insert into "${table}" (${sqlFieldList(schemas[schema].schema, table)}) values (?, ?)`);
    ...
    describe('readOne', () => {
        ...
        describe('Permissions, Authorization, Roles', () => {
            it.each(Object.keys(schemas))('%s it returns one item from tables as admin', async (schema) => {
                ...
                expect(tracker.history.select[0].sql).toBe(`select ${sqlFieldFormatter(schemas[schema].schema, table)} from "${table}" where "${table}"."id" = ? order by "${table}"."id" asc limit ?`);
    ...
    describe('readMany', () => {
        ...
        it.each(Object.keys(schemas))('%s it returns multiple items from tables as admin', async (schema) => {
            ...
            expect(tracker.history.select[0].sql).toBe(`select ${sqlFieldFormatter(schemas[schema].schema, table)} from "${table}" where ("${table}"."id" in (?, ?)) order by "${table}"."id" asc limit ?`);
        ...
        describe('Global Query Params', () => {
            it.each(Object.keys(schemas))(`Filter: %s _eq`, async (schema) => {
                ...
                expect(tracker.history.select[0].sql).toBe(`select ${sqlFieldFormatter(schemas[schema].schema, table)} from "${table}" where (1 = ? and "${table}"."id" = ?) order by "${table}"."id" asc limit ?`);
            ...
            it.each(Object.keys(schemas))(`Filter: %s _or`, async (schema) => {
                ...
                expect(tracker.history.select[0].sql).toBe(`select ${sqlFieldFormatter(schemas[schema].schema, table)} from "${table}" where (1 = ? and ("${table}"."id" = ? or "${table}"."name" = ?)) order by "${table}"."id" asc limit ?`);
                expect(response).toStrictEqual([items[1]]);
            });
        });
    });
    ...
    describe('updateBatch', () => {
        ...
        it.each(Object.keys(schemas))('%s batch update should only clear cache once', async (schema) => {
            ...
            const itemsServiceCacheClearSpy = vi.spyOn(itemsService.cache, 'clear').mockResolvedValue(() => vi.fn());
            await itemsService.updateBatch(items);
            expect(itemsServiceCacheClearSpy).toHaveBeenCalledOnce();
        });
        it.each(Object.keys(schemas))('%s batch update should throw InvalidPayloadException when passing non-array data', async (schema) => {
            const table = schemas[schema].tables[0];
            schemas[schema].accountability = null;
    ...
    describe('test filter queries', () => {
        ...
        it.each(Object.keys(schemas))('%s filters on top level', async (schema) => {
            ...
            expect(tracker.history.select[0].sql).toBe(`select "${table}"."id", "${table}"."name" from "${table}" where "${table}"."name" = ? order by "${table}"."id" asc limit ?`);
        });
        it.each(Object.keys(schemas))('%s filters on nested m2o level', async (schema) => {
            ...
            expect(tracker.history.select.length).toBe(1);
            expect(tracker.history.select[0].bindings).toStrictEqual(['something', 100]);
            expect(tracker.history.select[0].sql).toMatch(new RegExp(`select "${otherTable}"."id", "${otherTable}"."title" from "${otherTable}" ` +
        ...
        it.each(Object.keys(schemas))('%s filters on nested o2m level', async (schema) => {
            ...
            expect(tracker.history.select.length).toBe(1);
            expect(tracker.history.select[0].bindings).toStrictEqual(['something', 100]);
            expect(tracker.history.select[0].sql).toMatch(new RegExp(`select "${table}"."id", "${table}"."name" from "${table}" inner join ` +
    ...
    describe('test sort queries', () => {
        ...
        it.each(Object.keys(schemas))('%s sorts on top level', async (schema) => {
            ...
            expect(tracker.history.select[0].sql).toBe(`select "${table}"."id", "${table}"."name" from "${table}" order by "${table}"."name" asc limit ?`);
        });
        ...
        it.each(Object.keys(schemas))('%s sorts on nested o2m level', async (schema) => {
            ...
            expect(tracker.history.select[0].sql).toMatch(new RegExp(`select "${table}"."id", "${table}"."name" from "${table}" ` +
